...
class _GrammarManagementScreenState extends State<GrammarManagementScreen> {
    ...
    try {
      ...
      setState(() {
        _grammarList = data.map((g) => {
          ...
          'images': List<Map<String, dynamic>>.from(g['images'] ?? []),
          'audios': List<Map<String, dynamic>>.from(g['audios'] ?? []),
          'videos': List<Map<String, dynamic>>.from(g['videos'] ?? []),
        }).toList();
        _isLoading = false;
      });
    } catch (e) {
                ...
                children: [
                  Row(children: [
                    Container(
                      ...
                      child: const Icon(Icons.menu_book, color: Color(0xFF2196F3)),
                    ),
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      labelText: 'Tiêu đề *', hintText: 'VD: Present Simple Tense',
                  ...
                  TextField(
                    controller: theoryController, maxLines: 5,
                    decoration: InputDecoration(
                      labelText: 'Lý thuyết *', hintText: 'Nội dung lý thuyết ngữ pháp...',
                  ...
                  const SizedBox(height: 16),
                  TextField(
                    controller: examplesController, maxLines: 3,
                    decoration: InputDecoration(
                      labelText: 'Ví dụ (mỗi dòng 1 ví dụ)', hintText: 'I go to school every day.',
                      alignLabelWithHint: true,
                      border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)),
                      filled: true, fillColor: const Color(0xFFF8FAFC),
                    ),
                  ...
                  const Text('Chọn file hoặc nhập URL', style: TextStyle(fontSize: 12, color: Colors.grey)),
                  const SizedBox(height: 12),
                  _buildMediaSection(title: 'Hình ảnh', icon: Icons.image, color: Colors.blue, localPaths: imagePaths, urls: imageUrls,
                    onPickFile: () async {
                      try { final images = await ImagePicker().pickMultiImage(); if (images.isNotEmpty) setModalState(() => imagePaths.addAll(images.map((e) => e.path))); } catch (e) { _showError('Lỗi: $e'); }
                    },
                  ...
                  _buildMediaSection(title: 'Audio', icon: Icons.audiotrack, color: Colors.orange, localPaths: audioPaths, urls: audioUrls,
                    onPickFile: () async {
                      try { final r = await FilePicker.platform.pickFiles(type: FileType.audio, allowMultiple: true); if (r != null) setModalState(() => audioPaths.addAll(r.paths.whereType<String>())); } catch (e) { _showError('Lỗi: $e'); }
                    },
                  ...
                  _buildMediaSection(title: 'Video', icon: Icons.videocam, color: Colors.purple, localPaths: videoPaths, urls: videoUrls,
                    onPickFile: () async {
                      try { final r = await FilePicker.platform.pickFiles(type: FileType.video, allowMultiple: true); if (r != null) setModalState(() => videoPaths.addAll(r.paths.whereType<String>())); } catch (e) { _showError('Lỗi: $e'); }
                    },
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed: () async {
                        ...
                        if (result['error'] != null) _showError(result['error']); else { _showSuccess(grammar == null ? 'Tạo thành công!' : 'Cập nhật thành công!'); _loadGrammar(); }
                      },
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Text(title, style: TextStyle(fontWeight: FontWeight.w600, color: color)), const Spacer(),
          TextButton.icon(onPressed: onPickFile, icon: Icon(Icons.folder_open, size: 16, color: color), label: Text('File', style: TextStyle(color: color, fontSize: 12)), style: TextButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 8))),
          TextButton.icon(onPressed: onAddUrl, icon: Icon(Icons.link, size: 16, color: color), label: Text('URL', style: TextStyle(color: color, fontSize: 12)), style: TextButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 8))),
        ]),
        if (localPaths.isNotEmpty || urls.isNotEmpty) ...[
          const SizedBox(height: 8),
          Wrap(spacing: 8, runSpacing: 8, children: [
            ...localPaths.asMap().entries.map((e) => _buildChip(e.value.split('/').last, icon, color, false, () => onRemoveLocal(e.key))),
            ...urls.asMap().entries.map((e) => _buildChip(e.value.length > 20 ? '${e.value.substring(0, 20)}...' : e.value, Icons.link, color, true, () => onRemoveUrl(e.key))),
          ]),
        ],
      ]),
    );
  ...
  Widget _buildChip(String label, IconData icon, Color color, bool isUrl, VoidCallback onRemove) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
      decoration: BoxDecoration(color: isUrl ? color.withOpacity(0.2) : color.withOpacity(0.1), borderRadius: BorderRadius.circular(8), border: isUrl ? Border.all(color: color) : null),
      child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(icon, size: 14, color: color), const SizedBox(width: 4), Text(label.length > 12 ? '${label.substring(0, 12)}...' : label, style: TextStyle(fontSize: 11, color: color)), const SizedBox(width: 4), GestureDetector(onTap: onRemove, child: Icon(Icons.close, size: 14, color: color))]),
    );
  }
  ...
  Future<void> _showUrlInputDialog({required String title, required String hint, required Function(String) onAdd}) async {
    ...
    await showDialog(context: context, builder: (ctx) => AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        TextField(controller: c, decoration: InputDecoration(hintText: hint, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12))), keyboardType: TextInputType.url),
        const SizedBox(height: 8),
        Text('Hỗ trợ: URL (https://...) hoặc đường dẫn (D:/...)', style: TextStyle(fontSize: 11, color: Colors.grey[600])),
      ]),
      actions: [TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Hủy')), ElevatedButton(onPressed: () { final path = c.text.trim(); if (path.isNotEmpty) { onAdd(path); Navigator.pop(ctx); } else _showError('Vui lòng nhập đường dẫn!'); }, child: const Text('Thêm'))],
    ));
  }
  ...
  Widget _buildGrammarCard(Map<String, dynamic> grammar) {
    ...
    final hasMedia = (grammar['images'] as List).isNotEmpty || (grammar['audios'] as List).isNotEmpty || (grammar['videos'] as List).isNotEmpty;
    return Card(
      ...
      child: ExpansionTile(
        ...
        title: Row(children: [Expanded(child: Text(grammar['title'], style: const TextStyle(fontWeight: FontWeight.bold))), if (hasMedia) Icon(Icons.attach_file, size: 16, color: Colors.grey[500])]),
        ...
        children: [
          Padding(padding: const EdgeInsets.fromLTRB(16, 0, 16, 16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if (hasMedia) ...[const SizedBox(height: 12), Row(children: [if ((grammar['images'] as List).isNotEmpty) _buildBadge(Icons.image, Colors.blue, (grammar['images'] as List).length), if ((grammar['audios'] as List).isNotEmpty) _buildBadge(Icons.audiotrack, Colors.orange, (grammar['audios'] as List).length), if ((grammar['videos'] as List).isNotEmpty) _buildBadge(Icons.videocam, Colors.purple, (grammar['videos'] as List).length)])],
          ])),
        ],
      ),
    );
  ...
  Widget _buildBadge(IconData icon, Color color, int count) => Container(margin: const EdgeInsets.only(right: 6), padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: color.withOpacity(0.1), borderRadius: BorderRadius.circular(8)), child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(icon, size: 14, color: color), const SizedBox(width: 4), Text('$count', style: TextStyle(fontSize: 12, color: color, fontWeight: FontWeight.bold))]));
}
